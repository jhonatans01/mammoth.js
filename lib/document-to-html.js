...
function DocumentConversion(options, comments) {
    ...
    options = _.extend({ignoreEmptyParagraphs: false}, options);
    ...
    function convertParagraph(element, messages, options) {
        ...
        if (!element.alignment || element.alignment === "both") {
            paths.push(htmlPaths.element("p", {"align": "justify"}, {fresh: false}));
        } else {
            paths.push(htmlPaths.element("p", {"align": element.alignment}, {fresh: false}));
        }
        ...
        if (style) {
            ...
            stylePath = defaultParagraphStyle;
        }
        ...
        if (ignoreEmptyParagraphs) {
            ...
            return [Html.forceWrite].concat(nodes());
        }
    }
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.size) {
            paths.push(htmlPaths.element("span", {"style": 'font-size: ' + run.size + 'px'}, {fresh: true}));
        }
        var stylePath = htmlPaths.empty;
    ...
    function convertTable(element, messages, options) {
        tablePath = htmlPaths.element("table", {"width": element.width + '%'}, {fresh: true})
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.width !== 1) {
            attributes.width = element.width.toString() + '%';
        }
