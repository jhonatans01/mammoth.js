...
function BodyReader(options) {
    ...
    function sectionProperties(element) {
        if (element.first("w:sectPr")) {
            var sectPr = element.first("w:sectPr");
            if (sectPr) {
                return sectPr.first("w:pgNumType").attributes["w:start"];
            } else {
                return null;
            }
        }
    ...
    function readTableStyle(element) {
        return elementResultWithMessages({styleId: null, name: null, width: element.firstOrEmpty("w:tblW").attributes["w:w"]}, null);
    ...
    function readTableCell(element) {
        return readXmlElements(element.children).map(function(children) {
            ...
            var width = properties.firstOrEmpty("w:tcW").attributes["w:w"] ?
                 parseInt(properties.firstOrEmpty("w:tcW").attributes["w:w"]) / 50 : undefined;
